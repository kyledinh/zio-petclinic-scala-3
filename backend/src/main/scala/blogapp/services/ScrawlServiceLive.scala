...
final case class ScrawlServiceLive(dataSource: DataSource) extends ScrawlService {
  ...
  override def create(
    ...
    status: PubStatus, 
    userId: Uuid): Task[Scrawl] =
    for {
      scrawl <- Scrawl.make(title, body, createDate, status, userId)
  ...
  override def getForUser(userId: Uuid): Task[List[Scrawl]] =
    run(query[Scrawl].filter(_.userId == lift(userId)).sortBy(_.createDate))
      .provideEnvironment(ZEnvironment(dataSource))
